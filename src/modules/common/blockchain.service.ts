...
export class BlockchainService {
  ...
  async getAddress(coin: string, params = {}) {
    ...
    try {
      const response = await this.httpService
        .get(`${api}/${coin}/address`, {params})
        .toPromise();
      return response.data;
    } catch (e) {
      return this._forwardError(
        e,
        `Could not create new address coin=${coin} params=${JSON.stringify(
          params
        )}`
      );
    }
  ...
  async getTx(coin: string, txid: string) {
    ...
    try {
      ...
      return this._forwardError(
        e,
        `Could not get tx of coin=${coin} txid=${txid}`
      );
    }
  }
  ...
  async validateAddress(coin: string, address: string) {
    ...
    try {
      ...
      return this._forwardError(
        e,
        `Could not validate coin=${coin} address=${address}`
      );
    }
  }
  ...
  async createAdaAccount(coin: string, params: any) {
    ...
    try {
      const response = await this.httpService
        .get(`${api}/${coin}/account`, {params})
        .toPromise();
      return response.data;
    } catch (e) {
      return this._forwardError(
        e,
        `Could not create ADA account params=${JSON.stringify(params)}`
      );
    }
  }
  ...
  async getNormalizeAddress(coin: string, address: string) {
    ...
    try {
      ...
      return this._forwardError(
        e,
        `Could not convert ${address} to a checksum address.`
      );
    }
  }
  ...
  async _forwardError(e, errLog) {
    ...
    if (e.code === 'ECONNREFUSED') {
      ...
      throw Causes.BLOCKCHAIN_IS_NOT_STARTED;
    }
