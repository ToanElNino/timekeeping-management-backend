...
import firebaseAccountCredentials from '../../../push-notification-firebase.json';
...
export class NotificationService {
  ...
  async sendNotificationTopic(data: PushNotificationByTopic) {
    ...
    const payload = {
      notification: {
        title,
        body,
        // icon: 'https://cdnimg.vietnamplus.vn/uploaded/mzdic/2023_03_24/cristiano_ronaldo_portugal_2403.jpg',
      },
    };
    try {
      Promise.all([await admin.messaging().sendToTopic(topic, payload)]);
      // console.log(
      //   `${this.sendNotificationTopic.name} executed with notification payload`
      // );
      return data;
    } catch (error) {
      console.log(error);
  ...
  async sendNotificationByToken(data: PushNotificationByToken) {
    ...
    try {
      Promise.all([await admin.messaging().send(message)]);
      // console.log(
      //   `${this.sendNotificationByToken.name} executed with notification payload`
      // );
      return data;
    } catch (error) {
      console.log(error);
  ...
  async sendNotificationByUserWallet(data: PushNotificationByUserWallet) {
    const {title, body, userWallet} = data;
    const userDevice: UserDevice = await this.userDeviceRepo.findOne({
      where: {
    ...
    if (!userDevice) {
      throw new HttpException('user wallet is invalid', HttpStatus.BAD_GATEWAY);
    }
    const message = {
